...
// util function to normalize value into range
function normalize(value: number, min: number, max: number): number {
  let rval: number = null;
  if (min === undefined || max === undefined) {
    rval = value;
  } else {
    rval = (value - min) / (max - min);
  }
  return rval;
}

const TRAIN_DATA_PATH: string = path.join(__dirname, '../src/static/baseball/pitch_type_training_data.csv');
const TEST_DATA_PATH: string = path.join(__dirname, '../src/static/baseball/pitch_type_test_data.csv');
